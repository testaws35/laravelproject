...
class HomeController extends Controller
{
    ...
    public function changePassword(Request $request)
    {
        if (!(Hash::check($request->get('current-password'), Auth::user()->password))) 
        {
            ...
            return redirect()->back()->with("error",Lang::get('home.chng_pass_err'));
        }
        if(strcmp($request->get('current-password'), $request->get('new-password')) == 0)
        {
            //Current password and new password are same
            return redirect()->back()->with("error",Lang::get('home.chng_pass_err1'));
        }
        ...
        return redirect()->back()->with("success",Lang::get('passwords.password_changesuccess'));
        ...
        if(isset($request->key))
        {
                ...
                if (isset($searchResult) )    {
                    if( count($searchResult)>=1)
                    {
                        ...
                        $Failure = Lang::get('home.search_err');
            ...
            {
                $searchResult = null;
                $Failure = Lang::get('home.search_empty');
                return view('search',compact('searchResult', 'Failure'));
            }
